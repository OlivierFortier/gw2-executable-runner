...
#[cfg(feature = "nexus")]
use crate::nexus_addon::{manager::ExeManager, ui, NexusError, Result};
...
fn initialize_nexus_addon() -> Result<()> {
    ...
    let addon_dir = get_addon_dir("Nexus DX11 overlay loader")
        .ok_or_else(|| NexusError::ManagerInitialization("Failed to get addon directory".to_string()))?;

    fs::create_dir_all(&addon_dir)
        .map_err(|e| NexusError::ManagerInitialization(format!("Failed to create addon directory: {e}")))?;

    // Initialize the exe manager
    let exe_manager = std::sync::Arc::new(std::sync::Mutex::new(
        ExeManager::new(addon_dir)?
    ));

    crate::nexus_addon::manager::EXE_MANAGER.set(exe_manager.clone())
        .map_err(|_| NexusError::ManagerInitialization("Failed to set global exe manager".to_string()))?;
...
fn cleanup_nexus_addon() -> Result<()> {
    ...
    if let Some(exe_manager_arc) = crate::nexus_addon::manager::EXE_MANAGER.get() {
        let mut exe_manager = exe_manager_arc.lock()
            .map_err(|e| NexusError::ManagerInitialization(format!("Failed to lock exe manager during cleanup: {e}")))?;
